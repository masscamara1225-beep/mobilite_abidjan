    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          ...
          column(5,
                 card(title = "Niveau actuel par commune",
      ...
      tabItem(
        tabName = "carte",
        page_header(title = "Carte du réseau",
                    meta  = "Couleur des routes = niveau de congestion · Cliquez sur un arrêt pour le détail"),
        section_subtitle("Les chiffres deviennent géographie. Voyez physiquement où se forment
        ...
        sidebarLayout(
          ...
          mainPanel(
            width = 9,
            withSpinner(leafletOutput("carte_principale", height = 600),
      ...
      tabItem(
        tabName = "trafic",
        page_header(title = "Patterns de congestion",
                    meta  = "TomTom Traffic Flow · 10 axes · 5 jours · IC 95 % visualisés"),
        section_subtitle("Abidjan suit un rythme prévisible : deux pics quotidiens à 8 h et 17 h.
                          La zone bleue autour des courbes représente l'intervalle de confiance à 95 %."),
        tabsetPanel(
          id = "trafic_subtabs", type = "tabs",
          tabPanel("Courbe journalière", br(),
                   fluidRow(
                     ...
                     column(9,
                            withSpinner(plotlyOutput("courbe_journaliere", height = 420),
                     ...
                     column(3,
                            wellPanel(
                              ...
                              selectInput("heat_aggreg", "Agrégation",
                                          choices = c("Moyenne" = "mean", "Médiane" = "median", "Maximum" = "max"),
                                          selected = "mean")
                            )
                     ),
                     column(9,
                            withSpinner(plotlyOutput("heatmap_hebdo", height = 420),
                                        color = COULEURS$orange, type = 6))
                   )
          ),
          tabPanel("Comparateur d'axes", br(),
                   fluidRow(
                     ...
                     column(9,
                            withSpinner(plotlyOutput("barplot_pires", height = 420), type = 6))
                   )
          )
        )
      ),
      ...
      # ====================================================================
      # ONGLET 4 — EXPLORATION EDA
      # ====================================================================
      tabItem(
        tabName = "exploration",
        page_header(title = "Exploration statistique",
                    meta  = "Analyse exploratoire · IC 95 % par commune · Données flux_enrichi.csv"),
        section_subtitle("Au-delà des moyennes, les distributions et intervalles de confiance
        ...
        wellPanel(class = "well-min",
                  fluidRow(
                    ...
                    column(3, checkboxInput("explo_outliers", "Afficher les outliers", value = TRUE))
                  )
        ),
        fluidRow(
          column(6, card(title = "Distribution des vitesses par commune",
        ...
        fluidRow(
          column(12, card(title = "Statistiques descriptives par commune (avec IC 95 %)",
      ...
      tabItem(
        ...
        section_subtitle("Quelle commune paralyse tout Abidjan si elle est saturée ?
                          La théorie des graphes révèle les nœuds critiques."),
        fluidRow(
          column(3,
                 card(title = "Paramètres",
                      sliderInput("seuil_flux", "Seuil minimum de flux",
                                  min = 0, max = 100, value = 0, post = " obs"),
                      selectInput("metrique_choix", "Colorier par",
                                  choices = c("Communauté Louvain" = "groupe",
                                              "Degré"              = "degre")),
                      tags$hr(),
                      tags$p(class = "muted-small", textOutput("modularite")),
                      uiOutput("communautes_resume"))
          ),
          column(9,
                 card(withSpinner(visNetworkOutput("graphe_communes_vis", height = 480),
                                  color = COULEURS$vert, type = 6)))
        ),
        fluidRow(
          column(7, card(title = "Métriques par commune", DTOutput("tableau_metriques"))),
          column(5,
                 card(title = "Comment lire ce tableau ?",
                      tags$p(tags$strong("Intermédiarité (betweenness)"),
                             " — mesure combien de chemins passent par cette commune.
                       Une valeur élevée = commune critique."),
                      tags$p(tags$strong("Proximité (closeness)"),
                             " — mesure à quelle vitesse une commune peut atteindre toutes les autres."),
                      tags$p(tags$strong("Degré"),
                             " — nombre d'axes qui arrivent ou partent."),
                      tags$hr(),
                      note_box("Plateau concentre tous les flux entrants — c'est le hub central.
                        Si Adjamé est saturée, les flux vers Plateau sont coupés pour toute la rive nord.")))
        )
      ),
      
      # ====================================================================
      # ONGLET 6 — ML : 3 sous-pages
      # ====================================================================
      tabItem(
        tabName = "ml_pred",
        page_header(title = "Prédire un temps de trajet",
                    meta  = "Random Forest · OSRM · IC 95 % sur la prédiction"),
        fluidRow(
          column(4,
                 card(title = "Paramètres du trajet",
                      ...
                      dateInput("ml_date", "Date de départ",
          ...
          column(8,
                 ...
                 conditionalPanel(condition = "input.ml_predire == 0",
                                  tags$div(class = "empty-state",
                                           tags$p("Sélectionnez votre trajet et cliquez sur Prédire.")))
          )
        )
      ),
      ...
      tabItem(
        tabName = "ml_eval",
        page_header(title = "Performance des modèles",
      ...
      tabItem(
        tabName = "ml_clust",
        page_header(title = "Profils des communes",
                    meta  = "k-means · ACP · 3 clusters"),
        fluidRow(
          column(3, wellPanel(
            sliderInput("kmeans_k", "Nombre de clusters", min = 2, max = 6, value = 3)
          )),
      ...
      tabItem(
        tabName = "donnees",
        page_header(title = "Explorer les données brutes",
                    meta = "flux_enrichi · Communes · Arrêts GTFS · Téléchargement CSV"),
        section_subtitle("Transparence et reproductibilité : voici les données qui ont
      ...
      tabItem(
        ...
        page_header(title = "Recommandations actionnables",
                    meta  = "Synthèse pour les décideurs · ONG Abidjan Mobilité Durable"),
        section_subtitle("Cinq mesures concrètes, classées par impact attendu et faisabilité,
                          découlant directement des analyses précédentes."),
        fluidRow(
          column(12, card(title = "Tableau des recommandations",
